...
class DenoisingEval(kecam.backend.callbacks.Callback):
    def __init__(self, save_path, image_size=512, num_training_steps=1000, num_steps=50, rows=4, cols=4):
        super().__init__()
        self.save_path, self.image_size, self.num_training_steps, self.num_steps = save_path, image_size, num_training_steps, num_steps
        self.rows, self.cols, self.batch_size = rows, cols, rows * cols
...
class DefussionDatasetGen:
    def __init__(self, data_path, image_size=512, batch_size=32, num_training_steps=1000):
        ...
        self.all_images = self.walk_data_path_gather_images()
    ...
    def imread(self, image_path):
        return np.array(Image.open(image_path).resize([self.image_size, self.image_size], resample=Image.Resampling.BICUBIC)).astype("float32")
    ...
    def __next__(self):
        if self.generated < self.steps_per_epoch:
            images = self.all_images[self.generated * self.batch_size : (self.generated + 1) * self.batch_size]
...
def parse_arguments():
    ...
    parser.add_argument("-d", "--data_path", type=str, required=True, help="dataset directory containing images path")
...
if __name__ == "__main__":
    ...
    (xt, timestep), noise = next(iter(train_dataset_gen))
    print(">>>> Total train batches: {}".format(len(train_dataset_gen)))
    print(">>>> Data: xt.shape: {}, timestep.shape: {}, noise.shape: {}".format(xt.shape, timestep.shape, noise.shape))
    ...
    with global_strategy.scope():
        ...
        kwargs.update({"conditional_embedding": 0, "input_shape": xt.shape[1:]})  # Not using prompt conditional inputs
        ...
        if kecam.backend.is_torch_backend:
            ...
            train_dataset = build_torch_dataset(train_dataset_gen)
            ...
            train_dataset = build_tf_dataset(train_dataset_gen)
            compile_kwargs = {}
        # loss is mse(noise, model([xt, timestep]))
        model.compile(optimizer=optimizer, loss=kecam.backend.losses.MeanSquaredError(), **compile_kwargs)
        ...
        cols = args.batch_size // 4  # rows == 4
        eval_callback = DenoisingEval(os.path.join("checkpoints", basic_save_name), args.input_shape, num_training_steps=args.num_training_steps, cols=cols)
